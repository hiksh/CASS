...
```
data/raw/<dataset>/training-flow.csv
         │
         ▼
[Stage 1] 데이터 로드 & UDBB 샘플링
          데이터셋별 클래스 균형 설정 (config.py UDBB_COUNTS 참조)
         │
         ▼
[Stage 2] 전처리
...
[Stage 2.7] Reference Camouflage 참고값 계산 (비교용)
            literature baseline 첫 번째 항목(CICIDS2018: umar2024, UNSW-NB15: yin2023, Mirai: 없음)으로
...
| 비교군 | 선택 기준 | 피처 수 | 소스 풀 | 모델 편향 |
|--------|-----------|---------|---------|-----------|
| `cass` | UMAP Boundary_Mean 최대화 (silhouette > 0 제약) | N (자동) | pre-filter top-K | 없음 ← **제안 방법** |
| `anova` | ANOVA F-score 상위 N개 | N | **전체 피처** | 선형 분리도 가정 |
| `extratrees` | ExtraTrees 중요도 상위 N개 | N | **전체 피처** | 트리 구조 편향 |
| `random` | 무작위 N개 | N | **전체 피처** | — (하한 기준선) |
| `lit_<name>` | 논문 수동 선정 | 고정 | 전체 피처 | 도메인 전문가 편향 |

CASS는 pre-filter top-K 안에서 탐색하고, 나머지 비교군(anova/extratrees/random)은 전체 피처를 각자의 기준으로 독립 선택합니다.
...
![Pilot 검증 산점도](results/cicids2018/figures/pilot_fast_vs_full.png)
...
![2단계 스크리닝 플롯](results/cicids2018/figures/two_phase_screening.png)
...
![UMAP 최적 피처](results/cicids2018/figures/umap_best_subset.png)
...
![비교군 히트맵](results/cicids2018/figures/comparison_heatmap.png)
...
탐색 비용 절감 외에, ExtraTrees + ANOVA 평균 순위 결합은 두 기법의 편향(트리 구조 편향, 선형 분리도 가정)을 상호보완하여 통계적으로 무의미한 피처를 사전 제거한다. pre_filter는 **전체 피처 랭킹**을 반환하며, CASS는 상위 K개 안에서 탐색하고 anova/extratrees/random 비교군은 이 전체 랭킹에서 각자의 기준으로 독립 선택하여 선택 기준 자체의 차이를 순수하게 비교한다.
...
│   │   └── mirai/
│   │       ├── training-flow.csv      # Mirai 훈련 원본 (42 피처 + 레이블)
│   │       └── test-flow.csv          # Mirai 테스트 원본 (완전 분리 보관)
│   └── processed/
...
│   │   ├── logs/
│   │   └── exports/
│   ├── unsw_nb15/       # UNSW-NB15 실험 결과 (동일 구조)
│   │   ├── figures/
│   │   ├── logs/
│   │   └── exports/
│   └── mirai/           # Mirai 실험 결과 (동일 구조)
│       ├── figures/
│       ├── logs/
│       └── exports/
├── main.py              # CLI 진입점
...
**Mirai**
```
data/raw/mirai/training-flow.csv   # 피처 42개(수치형) + attack_flag + attack_step
data/raw/mirai/test-flow.csv       # 동일 컬럼 구조
```
`attack_step`: `reconnaissance` · `action` · `infection` (benign은 `-` 표기)

> **참고**: Mirai 데이터의 benign 행은 `attack_step = "-"`로 표기되어 있습니다.
> CASS는 `attack_flag == 0`으로 benign을 판별하므로 파이프라인에 영향 없습니다.
>
> 또한 단방향 플로우(backward 패킷 없음)의 경우 `backward_*` 컬럼 값이 전부 `-1`로 기록됩니다.
> 전처리 시 `-1 → NaN → median` 이후에도 NaN이 남는 컬럼(전체가 -1)은 `0`으로 대체합니다.
...
# ── Mirai ───────────────────────────────────────────────────
python main.py --dataset mirai --pilot --export --analyze

# ── 탐색 방식 변경 ──────────────────────────────────────────
python main.py --dataset mirai --mode random --n-subsets 100
python main.py --dataset mirai --top-k 15

# ── 개별 플래그 ─────────────────────────────────────────────
python main.py --dataset mirai --pilot    # Fast↔Full 상관 사전 확인
python main.py --dataset mirai --export   # 비교군 train/test CSV
python main.py --dataset mirai --analyze  # 8지표 히트맵
```
...
| 플래그 | 기본값 | 설명 |
|--------|--------|------|
| `--dataset` | `cicids2018` | 데이터셋 선택 (`cicids2018` / `unsw_nb15` / `mirai`) |
...
│   ├── logs/
│   │   ├── pre_filter_ranking.csv          # 전체 피처 랭킹 (ANOVA + ExtraTrees)
...
│   ├── logs/
│   └── exports/
└── mirai/               # --dataset mirai 결과 (동일 구조, lit_* 없음)
    ├── figures/
    ├── logs/
    └── exports/
```
...
① Inf / NaN → 열별 중앙값(median) 대체
   ...
   → 컬럼 전체가 -1/NaN인 경우(예: Mirai backward_* 단방향 플로우) fallback으로 0 대체
...
| 비교군 | 선택 방법 | 소스 풀 |
|--------|-----------|---------|
| `cass` | UMAP Boundary_Mean 최적화 결과 | Pre-filter top-K |
| `anova` | `filter_summary`를 ANOVA_F 내림차순 정렬 → 상위 N개 | **전체 피처 랭킹** |
| `extratrees` | `filter_summary`를 Tree_Importance 내림차순 정렬 → 상위 N개 | **전체 피처 랭킹** |
| `random` | `random.sample(feature_names, N)` — seed=RANDOM_SEED+1000 | **전체 피처** |
| `lit_<name>` | `LITERATURE_BASELINES[name]` 수동 정의 (데이터셋 내 존재하는 것만) | 전체 피처 |

> **설계 의도:** `pre_filter`는 전체 피처에 대한 ExtraTrees·ANOVA 점수를 계산하여 **전체 랭킹 DataFrame**을 반환합니다. CASS는 이 중 상위 K개 안에서 UMAP 기반 탐색을 수행하고, anova/extratrees/random 비교군은 전체 랭킹(혹은 전체 피처)에서 각자의 기준으로 독립 선택합니다. 이로써 CASS의 pre-filter 제약이 비교군에 유리하게 작용하는 편향을 제거하고, **선택 기준 자체의 차이**를 공정하게 비교합니다.
...
| 클래스 | 샘플 수 (상한) | 실제 보유량 |
|--------|--------------|------------|
| Benign | 10,000 | 38,586 |
| Reconnaissance | 1,617 | 1,617 |
| Action | 581 | 581 |
| Infection | 109 | 109 |

데이터 규모가 작아 상한을 넉넉히 설정하고, `min(상한, 실제 보유량)`으로 자동 제한됩니다. Benign:Attack 비율은 데이터 불균형으로 인해 다른 데이터셋과 다를 수 있습니다.
